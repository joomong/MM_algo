...
void GetUserStreamKeyThread(Json::Value& userResult)
{
    while (true)
    {
        Binanceusdm::GetUserStreamKey(userResult);
...
int main()
{
    ...
    Binanceusdm::init(API_KEY,SECRET_KEY);
    Binanceusdm::GetUserStreamKey(userResult);
    std::string userStreamKey = userResult["listenKey"].asString();
    std::string ws_path = "/ws/" + userStreamKey;
    std::string aggTrade_path =  "/ws/" + AvellanedaStrategy::ticker_ + "@aggTrade";
    std::string bookTicker_path = "/ws/" + AvellanedaStrategy::ticker_ + "@bookTicker";

    // Setting websocket endpoints
    Binanceusdm::connect_endpoint(bookTickerOnData,
                                  BINANCEUSDM_WS_HOST,
                                  BINANCEUSDM_WS_PORT,
                                  bookTicker_path.c_str());
    Binanceusdm::connect_endpoint(userStreamOnData,
                                  BINANCEUSDM_WS_HOST,
                                  BINANCEUSDM_WS_PORT,
                                  ws_path.c_str());
    Binanceusdm::connect_endpoint(aggTradeOnData,
                                BINANCEUSDM_WS_HOST,
                                BINANCEUSDM_WS_PORT,
                                aggTrade_path.c_str());

    // Start the websocket handler
    auto g = std::async(std::launch::async,Binanceusdm::enter_event_loop);
    ...
    while (true)
    {
        ...
        std::cout << "pnl: " << cash + normed_inventory * AvellanedaStrategy::order_quantity_ * mid_price << std::endl;
        std::cout << "=====================================" << std::endl;
        }
